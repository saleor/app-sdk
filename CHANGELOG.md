...
- c94ac69: Forward Dashboard-owned keyboard shortcuts out of the app iframe so global handlers (e.g. Cmd+K) keep working while the app has focus.
...
- 2d95899: Add the `openPopup` App Bridge action, the `OpenPopupParams` codec, an `identifier` field on manifest extensions, and `appParams` on AppBridge state.
...
- bc14ae9: Add `actions.RefreshEntity()` App Bridge action.
...
- 28a95b4: Updated manifest shape to match current Saleor schema. `appUrl` is no longer strictly required. Deprecated fields were removed (`configurationUrl`, `dataPrivacy`)
...
- 9580492: `next`, `react`, `react-dom`, `graphql` peer dependencies are now marked as optional, since only certain entry points require these dependencies.
