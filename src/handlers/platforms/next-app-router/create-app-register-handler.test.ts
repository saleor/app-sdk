...
describe("Next App Router createAppRegisterHandler", () => {
  const mockJwksValue = "{}";
  const mockAppId = "42";
  ...
  beforeEach(() => {
    ...
    request = new NextRequest("https://example.com", {
      ...
      headers: {
        ...
        Host: "mock-saleor-domain.saleor.cloud",
  ...
  it("Handles invalid JSON bodies", async () => {
    const brokenRequest = new NextRequest("https://example.com", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Host: "mock-saleor-domain.saleor.cloud",
  ...
  describe("Callback hooks", () => {
    ...
    it("Triggers success callbacks when APL save succeeds", async () => {
      ...
      expect(mockOnRequestStart).toHaveBeenCalledWith(
        request,
        expect.objectContaining({ authToken, saleorApiUrl }),
      );
      expect(mockOnRequestVerified).toHaveBeenCalledWith(
        request,
        expect.objectContaining({ authData: expectedAuthData }),
      );
    ...
    it("Triggers failure callback when APL save fails", async () => {
      ...
      expect(mockOnAuthAplFailed).toHaveBeenCalledWith(
        request,
        expect.objectContaining({ error: expect.any(Error), authData: expectedAuthData }),
      );
    });
    ...
    it("Allows custom error responses via hooks", async () => {
      const mockOnRequestStart = vi
        ...
        .mockImplementation((_req, context) =>
          context.respondWithError({ status: 401, message: "test message" }),
        );
      const handler = createAppRegisterHandler({
        apl: mockApl,
        onRequestStart: mockOnRequestStart,
      ...
      await expect(response.json()).resolves.toStrictEqual({
        error: { code: "REGISTER_HANDLER_HOOK_ERROR", message: "test message" },
        success: false,
      });
